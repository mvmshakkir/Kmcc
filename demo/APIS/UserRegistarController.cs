	...
	public class UserRegistarController : ControllerBase
	{
...

		{
			...
			if (ModelState.IsValid)
			{
				...
				{
					//bool existingUser = _context.Users.Any(u => u.Email == Reg.Email);
					bool existingUser = _context.Users.Any(u => u.Phone == Reg.Phone);
					if (existingUser)
					{
						string message = "Hello from API!";

						// Option 1: Using TempData
						//TempData["ApiMessage"] = message;

						// Option 2: Using ViewBag
						//ViewBag.ApiMessage = message;

						// Option 3: Using a custom model
						//var model = new MyModel { ApiMessage = message };
						//return Ok(model);
						...
						//TempData["AlertMessage"] = "Email already exists. Please use a different email.";
						var user = CreateUser();
						...
						if (Reg.PravasiStatus == "0")
						{
							user.PravasiStatus = PravasiStatus.Pravasi;
						}
						else
						{
							user.PravasiStatus = PravasiStatus.ExPravasi;
						}
                        ...
                        //{
                        //	var pravasistatus = Reg.PravasiStatus;
                        ...
                        //	}
                        //}
                        // Fetch the last RegistrationId from the database
                        //string lastRegistrationId = _context.demoUser
                        //    .OrderByDescending(p => p.RegistrationId)
                        //    .Select(p => p.RegistrationId)
                        //    .FirstOrDefault();
                        var registrationIds = _context.demoUser
      .Select(p => p.RegistrationId)
      .ToList();

                        //string lastRegistrationId = registrationIds
                        //    .OrderByDescending(id => int.TryParse(id, out int numericId) ? numericId : int.MinValue)
                        //    .ThenByDescending(id => id)
                        //    .FirstOrDefault();

						string lastRegistrationId = registrationIds
	.OrderByDescending(id => int.TryParse(Regex.Replace(id, @"^\D+", ""), out _)) // Prioritize numeric values after trimming non-numeric prefix
	.ThenByDescending(id => int.TryParse(Regex.Replace(id, @"^\D+", ""), out int numericId) ? numericId : int.MinValue) // Sort by numeric value if possible
	.ThenByDescending(id => id) // Sort alphanumeric IDs lexicographically if needed
	.FirstOrDefault();
						long lastIdNumericPart = 0; // To hold the numeric part of the last RegistrationId

                        // Determine the new RegistrationId
                        string newRegistrationId = "1000";

                        if (!string.IsNullOrEmpty(lastRegistrationId))
                        {
                            // Get the user's PravasiStatus
                            var pravasistatus = Reg.PravasiStatus;

                            // Extract the numeric part from the last RegistrationId
                            if (long.TryParse(lastRegistrationId.TrimStart('E'), out lastIdNumericPart)) // Remove 'E' if present
                            {
                                lastIdNumericPart++; // Increment the numeric part

                                // Check if the user is expravasi
                                if (pravasistatus == "1") // Adjust this condition based on your actual values
                                {
                                    // Prepend 'E' to the new RegistrationId
                                    newRegistrationId = "E" + lastIdNumericPart;
                                }
                                else
                                {
                                    // Normal RegistrationId for non-expravasi
                                    newRegistrationId = lastIdNumericPart.ToString();
                                }
                            }
                        }


                        // Now newRegistrationId contains the correct value based on the status

                        // Set the RegistrationId for the user
                        //user.RegistrationId = lastIdNumericPart.ToString();
                        user.RegistrationId = newRegistrationId;
                        var emailcheck = Reg.Email;

						await _userStore.SetUserNameAsync(user, Reg.Phone, CancellationToken.None);
						...
						if (result.Succeeded)
						{
							...
							if (Reg.family != null)
							{
								...
								await SendEmailAsync(Reg.Email, "Account Confirmation",
									$"Your kmcc account is successfully created.");
