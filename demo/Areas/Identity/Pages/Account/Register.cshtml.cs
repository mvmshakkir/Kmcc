...
namespace demo.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(RegisterModel demouser,string returnUrl = null)
        
        {
            returnUrl ??= Url.Content("~/");
            ExternalLogins = (await _signInManager.GetExternalAuthenticationSchemesAsync()).ToList();
            {
                if (demouser.Input.image != null)
                {
                    string folder = "profile/photo";
                    folder += demouser.Input.UserImage+Guid.NewGuid().ToString();
                    demouser.Input.UserImage = folder;
